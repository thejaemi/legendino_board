...
public class AnimationEventCopier : EditorWindow
{
    ...
    [MenuItem("Window/Animation Event Copier")]                            // Can load this via the window menu
    static void Init()
    {
    ...
    {
        //AnimationUtility.SetAnimationEvents(targetClip, null);                                                                // Remove all events
    ...
    void CopyAllData(GameObject source, GameObject target)
    {
        ...
        for (int i = 0; i < animationClipsSource.Length; i++)
        {                                                                // For each source Clip...
            bool foundMatch = false;                                                                                        // Check to make sure we found a match
            for (int n = 0; n < animationClipsTarget.Length; n++)
            {                                                            // ... & For each target clip...
                if (animationClipsSource[i].name == animationClipsTarget[n].name)
                {                                        // ...If the names match
